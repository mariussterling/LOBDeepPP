...
for f in {'___'.join(f.split('___')[:2]) for f in os.listdir(PATH_NN_MODEL)
          + os.listdir(PATH_NN_MODEL.replace('17a', '17b'))
          if 'z___' in f and model in f}:
    params, time_start = f.split('___')
    path_nn_model = PATH_NN_MODEL
    if params[:-1] == 'params_L0':
        path_nn_model = path_nn_model.replace('17a', '17b')
...
for k, v in yhats.items():
    with open(f'{path_save}/preds_L_yhats_{k}.json', 'w') as outfile:
        json_tricks.dump(v, outfile)
for k, v in ys_dict.items():
    with open(f'{path_save}/preds_L_ys_{k}.json', 'w') as outfile:
        json_tricks.dump(v, outfile)
...
for data_split in ['train', 'valid', 'test']:
    print(data_split)
    res = {}
    with open(f'{path_save}/preds_L_yhats_{data_split}.json') as json_file:
        yhats = json_tricks.load(json_file)
    with open(f'{path_save}/preds_L_ys_{data_split}.json') as json_file:
    ...
    with open(f'{path_save}/preds_L_errors_{data_split}.json', 'w') as outfile:
